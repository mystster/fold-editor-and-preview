        ...
        visit(tree, (node, index, parent) => {
            if (node.type === "html" && parent != null) {
                if (node.value.match(FOLD_START_KEYWORD)) {
                    ...
                    for (
                    //@ts-ignore
                    let index = parent.children.indexOf(node) + 1; index < parent.children.length; index++) {
                        const element = parent.children[index];
                        if (element.type === "html" && element.value.match(FOLD_START_KEYWORD)) {
                            keywordCount++;
                        }
                        else if (element.type === "html" && element.value.match(FOLD_END_KEYWORD)) {
                            keywordCount--;
                        }
...
module.exports = {
    ...
    handleEditorInit(editor) {
        ...
        cm.setOption("foldGutter", {
            rangeFinder: (codemirror, start) => {
                if (codemirror.getLine(start.line).match(FOLD_START_KEYWORD)) {
                    let keywordCount = 1;
                    for (let lineNo = start.line + 1; lineNo < codemirror.lineCount(); lineNo++) {
                        ...
                        if (keywordCount == 0) {
                            return {
                                from: CodeMirror.Pos(start.line, codemirror.getLine(start.line).length),
